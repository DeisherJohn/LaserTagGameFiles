...
if __name__ == '__main__':
    ...
    print 'reclone for friendly fire on? (y/[n])'
    ans = raw_input('')
    if len(ans) != 0:
        if ans[0] == 'y' or ans[0] == 'Y':
            os.system('ipython GunSettings/HungerGames.py')

    print 'enter chaser:'
    gun1 = raw_input('')
    gun1=int(gun1)

    print 'hit return to reveal the CHASER'
    os.system('ipython GunSettings/newGame.py %i' % gun1)

    print 'hit return to new game everyone'
    ans = raw_input('')
    os.system('ipython GunSettings/newGame.py')

    print '\n\nlight the RUNNERS up ... and start the game\n\n'
    ...
    try:
        while 1: 
            ...
            for ii in packetHandler.captures:
                if len(ii.frame.msdu) < 4: continue
                if ii.frame.msdu[2] == 32 or ii.frame.msdu[2] == 37: 
                    killed=ii.frame.msdu[3]
                    if killed in killList: continue
                    killList.append(killed)

                    # remove killed gun from gunlist
                    try:
                        del gunlist[gunlist.index(killed)]
                    except ValueError:
                        pass

                    print 'killed: %i (%i left)' % (len(killList),len(gunlist))
                
                if ii.frame.msdu[2] == 20 or ii.frame.msdu[2] == 17 or \
                    ii.frame.msdu[2] == 16 or ii.frame.msdu[2] == 21:
                    hit=ii.frame.msdu[3];hitter=ii.frame.msdu[4]
                    if (not hit in gunlist) and (not hit in killList): gunlist.append(hit)
                    if (not hitter in gunlist) and (not hitter in killList): 
                        gunlist.append(hitter)
