...
def analyzePackets(packetHandler):
    ...
    for gunNumber in range(50):
        ...
        for packet in packetHandler.captures:
            if len(packet.frame.msdu) < 5: continue
            if (packet.frame.msdu[2] == 32 or packet.frame.msdu[2] == 37) and \
                gunNumber == packet.frame.msdu[4]:
                # append time and gun killed to list
                morgue.append([packet.frame.msdu[3],packet.frame.timestamp])
        
        if len(morgue) == 0: continue # gun not in
        
        morgue.sort() # kills sorted by gun
        timeDeath=np.diff(np.array(morgue)[:,1]) # time between kills
...
if __name__ == '__main__':
    ...
    timeDelay=100
    try:
        for ii in range(timeDelay/10):
            time.sleep(10)
            print '%i seconds left' % ((9-ii)*timeDelay/10.)
            analyzePackets(packetHandler)
        
        os.system('ipython GunSettings/sniper.py')
        time.sleep(1)
        os.system('ipython GunSettings/newGame.py')
        for ii in range(timeDelay/10):
            time.sleep(10)
        ...
        os.system('ipython GunSettings/machineGun.py')
        time.sleep(1)
        os.system('ipython GunSettings/newGame.py')
        for ii in range(timeDelay/10):
            time.sleep(10)
    ...
    print 'reclone to vanilla settings? (y/[n])'
    ans = raw_input('')
    if len(ans) != 0:
