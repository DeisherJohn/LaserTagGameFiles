...
if __name__ == '__main__':
    ...
    print 'is this VIP juggernaut? (y/[n])'
    ans = raw_input('')
    if len(ans) != 0:
        if ans[0] == 'y' or ans[0] == 'Y':
            os.system('ipython GunSettings/VIPjuggernaut.py %i %i' % (gun1,gun2))

    # execute NEW GAME
    os.system('ipython GunSettings/newGame.py')

    print '\n\nLight sensors of VIP and start game ...\nNO NEW GAME ...\n\n'
    oldRed=0
    oldBlue=0
    elapsedTime=0
    flag60=True;flag90=True;flag120=True
    ...
    try:
        while 1: 
            time.sleep(0.1)
            elapsedTime += 0.1
            # if elapsedTime > 60 and flag60: 
            #     print "60 sec.: VIP on the move?"
            #     flag60=False
            # if elapsedTime > 90 and flag90: 
            #     print "90 sec.: VIP on the move?"
            #     flag90=False
            # if elapsedTime > 120 and flag120: 
            #     print "120 sec.: VIP on the move?"
            #     flag120=False
            blueHits=0
            redHits=0
            kills=0
            lastBlueHit=0;lastRedHit=0
            for ii in packetHandler.captures:
                if len(ii.frame.msdu) < 5: continue
                if (ii.frame.msdu[2] == 20 or ii.frame.msdu[2] == 17) and \
                    ii.frame.msdu[3] == redGun: 
                    redHits += 1
                    lastRedHit = ii.frame.msdu[4]

                if (ii.frame.msdu[2] == 20 or ii.frame.msdu[2] == 17) and \
                    ii.frame.msdu[3] == blueGun: 
                    blueHits += 1
                    lastBlueHit = ii.frame.msdu[4] 

                if (ii.frame.msdu[2] == 32 or ii.frame.msdu[2] == 37) and \
                    ii.frame.msdu[3] == redGun: 
                    print 'RED VIP is DEAD!  Blue wins!'
                    print 'kill shot by %i' % ii.frame.msdu[4]
                    os.system('ipython GunSettings/endGame.py')  
                    logger.info('Shutting down')
                    if snifferDev.isRunning():
                        snifferDev.stop()
                    print 'hit return to reclone to vanilla settings'
                    ans = raw_input('')
                    os.system('ipython GunSettings/vanilla.py')
                    sys.exit(0)

                if (ii.frame.msdu[2] == 32 or ii.frame.msdu[2] == 37) and \
                    ii.frame.msdu[3] == blueGun: 
                    blueHits += 1
                    print 'BLUE VIP is DEAD!  Red wins!'  
                    print 'kill shot by %i' % ii.frame.msdu[4]  
                    os.system('ipython GunSettings/endGame.py')  
                    if snifferDev.isRunning():
                        snifferDev.stop()
            ...
            if redHits != oldRed:
                print 'hit %i on RED by gun %i' % (redHits,lastRedHit)
                oldRed = redHits
            if blueHits != oldBlue:
                print 'hit %i on BLUE by gun %i' % (blueHits,lastBlueHit)
                oldBlue = blueHits
