...
def analyzePackets(packetHandler):
    ...
    for gunNumber in range(50):
        ...
        for packet in packetHandler.captures:
            try:
                if (packet.frame.msdu[2] == 32 or packet.frame.msdu[2] == 37) and \
                    gunNumber == packet.frame.msdu[3]:
                    # append time and gun killed to list
                    morgue.append(packet.frame.timestamp)
            except IndexError:
                pass
                    
        if len(morgue) == 0: continue # gun not killed
        
        morgue.sort() # kills sorted by gun
        timeDeath=np.diff(np.array(morgue)) # time between kills
        ...
        for kill in range(len(timeDeath)):
            if timeDeath[kill] < 100000: continue
            confirmedKills += 1
    ...
    return np.arange(50)[ind],results[ind]
...
if __name__ == '__main__':
    ...
    print('Team Death Match analyzer')
    ...
    try:
        while 1:
            if args.rude is True:
                ...
                try:
                    time.sleep(1)
                    gunNum,deaths=analyzePackets(packetHandler)
                    if np.sum(deaths) != oldDeaths:
                        oldDeaths=np.sum(deaths)
                        for ii in np.arange(len(gunNum)):
                            print 'gun %i has died %i times' % (gunNum[ii],deaths[ii])
                            if deaths[ii] == lives and (not gunNum[ii] in alreadyEnded):
                                os.system('ipython GunSettings/endGame.py %i' % gunNum[ii])
                                print 'sending endGame to %i' % gunNum[ii]
                                alreadyEnded.append(gunNum[ii])

                        ind=np.where((deaths == lives) & (gunNum % 2 == 1))[0]
                        print 'red guns eliminated: %i' % len(ind)
                        ind=np.where((deaths == lives) & (gunNum % 2 == 0))[0]
                        print 'blue guns eliminated: %i' % len(ind)
